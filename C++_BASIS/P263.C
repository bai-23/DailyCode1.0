...
int Sum1(int arr[], int n1, int *p1);

int main(void)
{
	int arr1[6], arr2[10], arr3[15], gt60A, gt60B, gt60C, sumA, sumB, sumC;
	
	sumA = Sum1(arr1, 6, &gt60A);  /* userCode(<60字符): 调用函数读 6个数到arr1中，并计算和及>60的个数 */
	sumB = Sum1(arr2, 10, &gt60B);  /* userCode(<60字符): 调用函数读10个数到arr2中，并计算和及>60的个数 */
	sumC = Sum1(arr3, 15, &gt60C);  /* userCode(<60字符): 调用函数读15个数到arr3中，并计算和及>60的个数 */
